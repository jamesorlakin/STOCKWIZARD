...
include('header.htm');
echo '<p><a href="locationsshowlist.php?id=' . $_GET['locationid'] . '">Back to location</a> | Search in \'' . $_GET['locationname'] . '\':</p>';
echo '<form>
...
echo '<option value="dateoftransfer" ' . $dateo . '>Date</option>';
...
<input type="hidden" name="locationname" value="' . $_GET['locationname'] . '">
...
$q = "SELECT * FROM simlocationslist WHERE " . $_GET['searchwhat'] . " LIKE '%" . $_GET['query'] . "%' AND locationid = '{$_GET['locationid']}' ORDER BY supplier ASC, dateofpurchase DESC";
} else {
$date1 = $_GET['date1year'] . '-' . $_GET['date1month'] . '-' . $_GET['date1day'];
$date2 = $_GET['date2year'] . '-' . $_GET['date2month'] . '-' . $_GET['date2day'];
$q = "SELECT * FROM simlocationslist WHERE dateofpurchase BETWEEN '$date1' AND '$date2' AND locationid = '{$_GET['locationid']}' ORDER BY supplier ASC, dateofpurchase DESC";
}
...
echo '<table border="1">
	<tr>
		...
		echo '>Sales price</td>
		<td>Margin</td>
		<td colspan="2">Editing options</td>
	</tr>';
	$totalstock = 0;
	$counter = 1;
...
echo '<td><a name="list' . $counter . '" href="locationsedit.php?id=' . $cr['id'] . '&list=' . $counter . '&search=yes&searchwhat=' . $_GET['searchwhat'] . '&query=' . $_GET['query'] . '">Edit</a></td>
<td><a href="locationsdelete.php?id=' . $cr['id'] . '">Delete</a></td></tr>';
	$counter = $counter + 1;
}
